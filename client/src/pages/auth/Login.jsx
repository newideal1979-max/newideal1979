...
export default function Login() {
  const { login, loginWithGoogle, firebaseUser } = useAuth();
  ...
  // Navigating here — instead of right after login()/loginWithGoogle() resolve — means we
  // only ever redirect once AuthContext's firebaseUser is actually set. That closes the gap
  // where ProtectedRoute could see a stale "no user yet" and bounce back to /login.
  useEffect(() => {
    if (firebaseUser) navigate(redirect, { replace: true });
  }, [firebaseUser, redirect, navigate]);
